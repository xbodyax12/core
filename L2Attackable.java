...
public class L2Attackable extends L2NpcInstance {
     ...
     */
    private final Map<L2Character, AggroInfo> _aggroList = new SingletonMap<L2Character, AggroInfo>().shared("L2Attackable._aggroList");
    ...
    private final Map<L2Character, AggroInfo> _damageContributors = new SingletonMap<L2Character, AggroInfo>().shared("L2Attackable._damageContributors");
     ...
     */
    private final Map<L2PcInstance, AbsorberInfo> _absorbersList = new SingletonMap<L2PcInstance, AbsorberInfo>().shared("L2Attackable._absorbersList");
    /**
     ...
     * to NPC_STD_CALCULATOR)</li> <li>Set the name of the L2Attackable</li>
     * <li>
     ...
     */
    private long _dieTime = 0;

    public boolean passFarmDelayCheck() {
        return (isGrandBoss() || isRaid() || isBoss());
    }
    ...
    public boolean doDie(L2Character killer) {
        ...
        try {
            if (killer != null) {
                ...
                if (player != null && !player.hasFarmDelay(this)) {
        ...
        /*if (killer instanceof L2PlayableInstance && ((L2PlayableInstance) killer).isInJailMission()) {
         ...
         killer.sendPacket(new PlaySound(0, "ItemSound.quest_middle", 0, killer.getObjectId(), killer.getX(), killer.getY(), killer.getZ()));
         ...
         killer.sendPacket(new PlaySound(0, "ItemSound.quest_accept", 0, killer.getObjectId(), killer.getX(), killer.getY(), killer.getZ()));
         }
         }*/
        //L2EMU_ADD_END
        setChampion(false);
        castDeathSkill(getKnownSkill(4614), killer);

        if (Config.FARM_DELAY && !passFarmDelayCheck()) {
            checkFarmDelay(killer.getPlayer());
        }
        return true;
    }

    private void checkFarmDelay(L2PcInstance player) {
        if (player == null) {
            return;
        }

        ServerData.getInstance().checkFarmDelay(player);
    }

    private void castDeathSkill(L2Skill skill, L2Character killer) {
        if (skill == null || killer == null) {
            return;
        }

        int damage = (int) Formulas.calcPhysDam(this, killer, skill, (byte) 0, false, false, false);
        killer.reduceCurrentHp(damage, this, skill);
        broadcastPacket(new MagicSkillUse(this, this, 4614, 1, skill.getHitTime(), 1, false));
    }
    ...
    protected void calculateRewards(L2Character lastAttacker) {
        if (lastAttacker.hasFarmDelay(this)
                || EventDroplist.hasDropPenalty(getNpcId())) {
            return;
        }
        EventDroplist.setDayDropPenalty(getNpcId());
...
//							rewardCount++;
                ...
                for (FastMap.Entry<L2Character, RewardInfo> entry = rewards.head(), end = rewards.tail(); (entry = entry.getNext()) != end;) {
                    ...
                    if (attackerParty == null) {
                        ...
                        if (attacker.getKnownList().knowsObject(this)) {
                            ...
                            if (isChampion()) {
                                if (isChampion2()) {
                                    exp *= Config.CHAMPION_EXP_SP2;
                                    sp *= Config.CHAMPION_EXP_SP2;
                                } else {
                        ...
                        if (isChampion()) {
                            if (isChampion2()) {
                                exp *= Config.CHAMPION_EXP_SP2;
                                sp *= Config.CHAMPION_EXP_SP2;
                                exp_premium *= Config.CHAMPION_EXP_SP2;
                                sp_premium *= Config.CHAMPION_EXP_SP2;
                            } else {
                                exp *= Config.CHAMPION_EXP_SP;
                                sp *= Config.CHAMPION_EXP_SP;
    ...
    private RewardItem calculateRewardItem(L2PcInstance lastAttacker, L2DropData drop, int levelModifier, boolean isSweep) {
        ...
        if (isChampion() && !drop.isSealStone()) {
            if (drop.getItemId() == 57 || drop.getItemId() == 5575) {
                champRate = isChampion2() ? Config.CHAMPION_ADENA2 : Config.CHAMPION_ADENA;
            } else {
                champRate = isChampion2() ? Config.CHAMPION_REWARDS2 : Config.CHAMPION_REWARDS;
            }
        } else {
            champRate = 1;
        }
        ...
        if (Rnd.get(L2DropData.MAX_CHANCE) < categoryDropChance) {
            ...
            if (isChampion()) {
                if (drop.isSealStone()) {
                    ...
                    champRate = isChampion2() ? Config.RATE_DROP_SEAL_CHAMPIONS2 : Config.RATE_DROP_SEAL_CHAMPIONS;
                }
            } else {
                champRate = 1;
            }
            ...
            //int rnd = Rnd.get(L2DropData.MAX_CHANCE);
            if (drop.isSafeChance()) {
                dropChance = drop.getChance();
                //lastAttacker.sendCritMessage("##" + rnd + "/" + dropChance);
            }
            if (drop.isFixedDropCount() && Rnd.get(L2DropData.MAX_CHANCE) < dropChance) {
                return new RewardItem(drop.getItemId(), Rnd.get(min, max));
            }
    ...
    @Override
    public boolean canBeSweepped(L2PcInstance speeper) {
        if (!isDead()) {
            return false;
        }

        if (!isSpoil()) {
            speeper.sendPacket(SystemMessageId.SWEEPER_FAILED_TARGET_NOT_SPOILED);
            speeper.sendPacket(ActionFailed.STATIC_PACKET);
            return false;
        }

        if (System.currentTimeMillis() > _dieTime + 10000) {
            return true;
        }

        if (speeper.getObjectId() != getIsSpoiledBy() && !speeper.isInLooterParty(getIsSpoiledBy())) {
            speeper.sendPacket(SystemMessageId.SWEEP_NOT_ALLOWED);
            speeper.sendPacket(ActionFailed.STATIC_PACKET);
            return false;
        }
    ...
    public void doItemDrop(L2NpcTemplate npcTemplate, L2Character lastAttacker) {
        ...
        L2PcInstance player = lastAttacker.getPlayer();//.getActingPlayer();
                ...
                else
                {
                    ...
                    if (item != null)
                    {
                        ...
                        switch (item.getItemId())
                        {
                            ...
                            case 6361:
                            case 6362:
                                if ((Config.AUTO_LOOT_RAID && isRaid() && player.isAutoLootEnabled()) || player.isAutoLootEnabled() && !isRaid())
        ...
        if (isChampion()) {
            int chLevelDiff = isChampion2() ? Config.CHAMPION_SPCL_LVL_DIFF2 : Config.CHAMPION_SPCL_LVL_DIFF;
            int chSpclChance = isChampion2() ? Config.CHAMPION_SPCL_CHANCE2 : Config.CHAMPION_SPCL_CHANCE;
            int chSpclQty = isChampion2() ? Config.CHAMPION_SPCL_QTY2 : Config.CHAMPION_SPCL_QTY;
            int chSpclItem = isChampion2() ? Config.CHAMPION_SPCL_ITEM2 : Config.CHAMPION_SPCL_ITEM;

            if (Math.abs(getLevel() - player.getLevel()) <= chLevelDiff && chSpclChance > 0
                    && Rnd.get(100) < chSpclChance) {
                int champqty = Rnd.get(chSpclQty) + 1; //quantity should actually vary between 1 and whatever admin specified as max, inclusive.

                // Give this or these Item(s) to the L2PcInstance that has killed the L2Attackable
                RewardItem item = new RewardItem(chSpclItem, champqty);
                if (player.isAutoLootEnabled(item.getItemId())) {
...
/*			random = Rnd.get(100);
    ...
    public L2ItemInstance dropItem(L2PcInstance lastAttacker, RewardItem item) {
        ...
        for (int i = 0; i < item.getCount(); i++) {
            ...
            //int newZ = GeoData.getInstance().getSpawnHeight(newX, newY, getZ());
    ...
    /**
     * РџРѕРІРµСЂРєР° РјРѕР¶РЅРѕ Р»Рё СЃРЅСЏС‚СЊ РҐРџ<br>
     *
     * @param damage as double - РЅР°РЅРµСЃРµРЅРЅС‹Р№ СѓСЂРѕРЅ<br>
     * @param attacker as L2Character - РЅР°РЅРµСЃС€РёР№ РґР°РјР°Рі<br>
     * @return as boolean - РІРѕР·РјРѕР¶РЅРѕСЃС‚СЊ РЅР°РЅРµСЃС‚Рё СѓСЂРѕРЅ
     */

...
public class L2NpcInstance extends L2Character {
    ...
    /**
     * Task launching the function onRandomAnimation() Scheduled for
     * L2MonsterInstance only if AllowRandomAnimation=true
     */
     ...
     * calculated delay if the server allow it </li><BR><BR>
     *
     ...
     * range will help it if it's attacked<BR><BR>
     *
     */
    ...
    protected boolean canTarget(L2PcInstance player) {
        ...
        if (Config.GEODATA > 0 && checkSeeTarget(getTemplate().getNpcId()) && !GeoData.getInstance().canSeeTarget(player, this, getInstanceId())) {
            player.sendMessage("Вы должны находиться ближе к цели.");
            player.sendPacket(ActionFailed.STATIC_PACKET);
            return false;
        }
    ...
    private boolean checkSeeTarget(int id) {
        if (isRaid() || isGrandBoss()) {
            return true;
        }
        if (id == 29025) {
            return true;
     ...
     * L2NpcInstance position and heading on the client </li><BR><BR>
     *
                    ...
                    if (!canInteract(player)) {
                        ...
                        }
                        {
                            ...
                            if ((qlst != null) && qlst.length == 1) {
                                ...
                                showChatWindow(player, getNpcId() == 31521 ? 777 : 0);
                            }
                        }
                        player.sendPacket(ActionFailed.STATIC_PACKET);
                    }
    ...
    public void onActionShift(L2PcInstance player) {
        if (player.isGM()) {
            ...
            NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
            TextBuilder html1 = new TextBuilder("<html><title>Инофрмация о Дропе, Спойле</title><body>");

			if (Config.ALT_GAME_SHOWPC_DROP)
			{
            html1.append("<table border=1 width=\"100%\">");
			html1.append("<tr><td><center><font color=\"CC9966\">Предмет</font></center></td><td width=\"55\" valign=middle align=center><center><font color=\"CC9966\">Тип</font></center></td><td width=\"50\" valign=middle align=center><center><font color=\"CC9966\">Шанс</font></center></td></tr>");
	        ...
	        if (getTemplate().getDropData() != null)
	            for (L2DropCategory cat : getTemplate().getDropData())
	                for (L2DropData drop : cat.getAllDrops())
	                {
	                    ...
	                    html1.append("<tr><td><font color=\"LEVEL\">" + name + "</font></td><td width=\"55\" valign=middle align=center>" + (drop.isQuestDrop() ? "<font color=\"FF6600\">Квест</font>" : (cat.isSweep() ? "<font color=\"71C2FF\">Спойл</font>" : "<font color=\"00FF00\">Дроп</font>")) + "</td><td width=\"55\" valign=middle align=center>" + (drop.getChance() >= 10000 ? (double)drop.getChance() / 10000 : drop.getChance() < 10000 ? (double)drop.getChance() / 10000 : "N/A") + "%</td></tr>");
	                }
    ...
    public void onBypassFeedback(L2PcInstance player, String command) {
        //if (canInteract(player))
        {
            ...
            // [J2J_JP ADD END]
                ...
                try {
                    player.setChatLanguage(Integer.parseInt(command.substring(5)));
                } catch (Exception e) {
                }
                showChatWindow(player, getHtmlPath(getTemplate().getNpcId(), 0, player));
            } else if (command.startsWith("listTraders")) {
                OfflineManager.getInstance().showTraders(player, getObjectId());
            } else if (command.startsWith("showTrader")) {
                int charId = 0;
                try {
                    charId = Integer.parseInt(command.substring(11));
                } catch (Exception e) {
                    charId = 0;
                }
                OfflineManager.getInstance().showTradeList(player, charId, getObjectId(), "", "");
            } else if (command.startsWith("showSearch")) {
                String type = "";
                try {
                    type = command.substring(11);
                } catch (Exception e) {
                    type = "";
                }
                OfflineManager.getInstance().showSearch(player, getObjectId(), type);
            } else if (command.startsWith("processSearch")) {
                String type = "";
                String value = "";
                try {
                    String[] data = command.substring(14).split(" _&x_ ");
                    type = data[0];
                    value = data[1];
                } catch (Exception e) {
                    type = "none";
                }
                OfflineManager.getInstance().processSearch(player, getObjectId(), type, value);
            } else if (command.startsWith("showFindTrader")) {
                int charId = 0;
                String type = "";
                String value = "";
                try {
                    String[] data = command.substring(15).split(" _&x_ ");
                    charId = Integer.parseInt(data[0]);
                    type = data[1];
                    value = data[2];
                } catch (Exception e) {
                    charId = 0;
                ...
                OfflineManager.getInstance().showTradeList(player, charId, getObjectId(), type, value);
            } else if (command.startsWith("followTrader")) {
                int charId = 0;
                try {
                    charId = Integer.parseInt(command.substring(13));
                } catch (Exception e) {
                    charId = 0;
                }
                OfflineManager.getInstance().followTrader(player, charId);
            } else if (command.startsWith("showItemTrader")) {
                int charId = 0;
                int itemId = 0;
                try {
                    String[] data = command.substring(15).split("_");
                    charId = Integer.parseInt(data[0]);
                    itemId = Integer.parseInt(data[1]);
                } catch (Exception e) {
                    charId = 0;
                }
                OfflineManager.getInstance().showItemTrader(player, getObjectId(), charId, itemId);
            }
        }
        player.sendPacket(ActionFailed.STATIC_PACKET);
    }
     ...
     * L2NpcInstance to the L2PcInstance </li><BR><BR>
     *
     ...
     * avoid that the client wait another packet </li><BR><BR>
     *
     * @param player The L2PcInstance that talk with the L2NpcInstance
     ...
     *
     * <FONT COLOR=#FF0000><B> Newbie Helper Buff list is define in buff
     * templates sql table as "SupportMagic"</B></FONT><BR><BR>
     *
     * @param player The L2PcInstance that talk with the L2NpcInstance
     *
     */
    ...
    public void showChatWindow(L2PcInstance player, int val) {
        ...
        switch (npcId) {
            ...
            case 31521:
                if (val == 777) {
                    player.setQuestKey();
                    filename = "data/html/custom/jeremy.htm";
                }
                break;
            default:
        ...
        html.replace("%alrb_resp%", getRaidBossDeath(0));
        html.replace("%objectId%", String.valueOf(getObjectId()));

        if (npcId == 31521
                && val == 777) {
            html.replace("%quest_protect%", player.getQuestKey());
        }
    ...
    public String getRaidBossDeath(int id) {
        return "";
    }

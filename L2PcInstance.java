...
import ru.catssoftware.gameserver.handler.itemhandlers.Potions;
...
import ru.catssoftware.gameserver.instancemanager.BuffStoreManager;
import ru.catssoftware.gameserver.instancemanager.BuffStoreManager.StoredSkill;
import ru.catssoftware.gameserver.instancemanager.LogoutManager;
import ru.catssoftware.gameserver.model.BypassManager;
...
import ru.catssoftware.gameserver.util.SmsEngine;
...
//	private boolean 		_isWinLastHero 						= false;
    ...
    private static final String CHAR_SETTINGS = "SELECT enddate FROM character_herolist WHERE charId=?";

    private static final String PREM_SETTINGS = "SELECT valueData FROM account_data WHERE account_name=? AND valueName=?";
    ...
    protected Map<Integer, L2CubicInstance> _cubics = new SingletonMap<Integer, L2CubicInstance>().shared("L2PcInstance._cubics");
    /**
     * The L2FolkInstance corresponding to the last Folk which one the player
     * talked.
     */
    private L2FolkInstance _lastFolkNpc = null;
    protected final Map<Integer, Integer> _activeSoulShots = new SingletonMap<Integer, Integer>().shared("L2PcInstance._activeSoulShots");
    ...
    public static void disconnectIfOnline(int objectId) {
        ...
        if (player.isOfflineTrade()) {
            OfflineManager.getInstance().removeTrader(player);
        }
    ...
    //L2EMU_EDIT
    /**
     * Check if the L2PcInstance already have take the itemId for the
     * questId.<BR><BR>
     */
    ...
    /**
     * Return a table containing all Common L2Recipe of the
     * L2PcInstance.<BR><BR>
     */
    ...
    /**
     * Add a new L2Recipe to the table _commonrecipebook containing all L2Recipe
     * of the L2PcInstance <BR><BR>
     *
    ...
    /**
     * Add a new L2Recipe to the table _recipebook containing all L2Recipe of
    ...
    /**
     * @param RecipeID The Identifier of the L2Recipe to check in the player's
     * recipe books
     *
     * @return
     * <b>TRUE</b> if player has the recipe on Common or Dwarven Recipe book
    ...
    /**
     * Tries to remove a L2Recipe from the table _DwarvenRecipeBook or from
     * table _CommonRecipeBook, those table contain all L2Recipe of the
     * L2PcInstance <BR><BR>
     *
     * @param RecipeID The Identifier of the L2Recipe to remove from the
     * _recipebook
     *
     */
    ...
    /**
     * Return the number of recommendation that the L2PcInstance can
     * give.<BR><BR>
     */
    ...
    /**
     * Increment the number of recommendation that the L2PcInstance can
     * give.<BR><BR>
     */
    ...
    public void academyCheck(int Id) {
        ...
        if ((getSubPledgeType() == -1 || getLvlJoinedAcademy() != 0) && canLeaveAcademy(Id)) {
            _lastAcademyCheck = 1;

            if (Config.CLAN_ACADEMY_KICK_LEVEL == 0) {
                if (getLvlJoinedAcademy() <= 16) {
                    _clan.increaseRepScrore("Academy", 650, true);
                } else if (getLvlJoinedAcademy() >= 39) {
                    _clan.increaseRepScrore("Academy", 190, true);
                } else {
                    _clan.increaseRepScrore("Academy", (650 - (getLvlJoinedAcademy() - 16) * 20), true);
                }
            }
    ...
    public boolean canLeaveAcademy(int Id) {
        if (Config.CLAN_ACADEMY_KICK_LEVEL > 0) {
            return getLevel() >= Config.CLAN_ACADEMY_KICK_LEVEL;
        }

        return (PlayerClass.values()[Id].getLevel() == ClassLevel.Third && !isSubClassActive());
    }

    public boolean forbidJoinAcademy() {
        if (Config.CLAN_ACADEMY_KICK_LEVEL > 0) {
            return getLevel() >= Config.CLAN_ACADEMY_KICK_LEVEL;
        }

        return (getLevel() > 40 || getClassId().level() >= 2);
    }
     ...
     *
     */
    public void rewardSkills(boolean lvlup) {
        if (!lvlup) {
    ...
    /**
     * While animation is shown, you may NOT move/use skills/sit/stand again in
     * retail.<BR><BR>
     *
     * @author SaveGame
     */
    ...
    private void processAddItem(boolean UpdateIL, L2ItemInstance newitem) {
        ...
        if (!isGM() && !getInventory().validateCapacity(0)) {
            ...
            if (handler == null) {
                ...
                handler.useItem(this, newitem, 0);
            }
            return;
        }
    ...
    private int _bossZone = 0;

    @Override
    public void setInBossZone(int inside) {
        _bossZone = inside;
    }

    public int getBossZone() {
        return _bossZone;
    }

    public boolean isInBossZone() {
        return (_bossZone != 0);
    }

    private boolean canTargetBossZone(L2PcInstance player) {
        if (isInAqZone() || player.isInAqZone()) {
            return true;
        }

        return getBossZone() == player.getBossZone();
    }
    ...
    public void onAction(L2PcInstance player) {
        ...
        //if (getBossZone() != player.getBossZone()) {
        if (!canTargetBossZone(player)) {
    ...
    public final void broadcastStatusUpdateImpl() {
        ...
        broadcastStatusToPartyMembers(getParty());
    ...
    private void broadcastStatusToPartyMembers(L2Party party) {
        if (party == null) {
            return;
        }

        if (needHpUpdate(352) || needMpUpdate(352) || needCpUpdate(352)) {
    ...
    public void updateEffectIconsImpl() {
        ...
        if (Config.OVB_TEST) {
            ...
            ConcurrentLinkedQueue<String> tested = new ConcurrentLinkedQueue<String>();
            for (L2Effect effect : getAllEffects()) {
                if (effect == null) {
                    continue;
                }
                if (tested.contains(effect.getStackType())) {
                    try {
                        stopSkillEffects(effect.getId());
                    } catch (Exception e) {
                    }
                    continue;
                }
                tested.add(effect.getStackType());
            }
        }
...
//		Broadcast.toKnownPlayers(this, new NicknameChanged(this));
    ...
    public void doAutoLoot(L2Attackable target, L2Attackable.RewardItem item) {
        ...
        if (getActiveWarehouse() != null) {
            cancelActiveWarehouse();
        }
        if (isInParty()) {
...
//		sendPacket(sm);
        ...
        if (Config.PERMA_LOG_COIN && target.getItemId() == 4037) {
            Log.add("#" + target.getCount() + "#" + getFingerPrints(), "item/doPickupItem");
        }
     ...
     * _knownObject and L2PcInstance to _KnownObject of the
     * L2Object)</li><BR><BR>
     *
    ...
    public void setTarget(L2Object newTarget) {
        ...
        //L2EMU_EDIT
        if (oldTarget != null && newTarget != null) //L2EMU_EDIT
        {
    ...
    public void onPvPPkKill(L2Character killer) {
        ...
        if ((pk != null && pk.isInFlagZone()) && isInFlagZone()) {
            return;
        }
        ...
        if (!srcInPvP) {
            ...
            if (pk != null) {
                ...
                if (clanWarKill) {
                    ...
                    if (_clan.getReputationScore() >= Config.ALT_REPUTATION_SCORE_PER_KILL) {

                        _clan.decreaseRepScrore(Config.ALT_REPUTATION_SCORE_PER_KILL, true);
                        pk.getClan().increaseRepScrore("onPvPPkKill", Config.ALT_REPUTATION_SCORE_PER_KILL, true);
     ...
     * <li>Manage Karma gain for attacker and Karma loss for the killed
     * L2PcInstance </li>
    ...
    public boolean doDie(L2Character killer) {
        ...
        if (Config.DISCO_MOD && _bossLogout) {
            OfflineManager.getInstance().kickPlayer(this);
        }
    ...
    private void onDieUpdateKarma() {
        if (getKarma() > 0) {
            ...
            SystemMessage msg = new SystemMessage(SystemMessageId.YOUR_KARMA_HAS_BEEN_CHANGED_TO_S1);
            msg.addNumber(getKarma());
            sendPacket(msg);
            broadcastFullInfo();
        }
    }
    ...
    public void onKillUpdatePvPKarma(L2Character target) {
        ...
        if (Config.PVP_AOE_BONUS
                && targetPlayer.isInAoeZone()
                && isInAoeZone()) {
        ...
        if (targetPlayer.isInFlagZone() && isInFlagZone()) {
            _nowKilled = targetPlayer.getObjectId();
            increasePvpKills(targetPlayer.getLevel());
            return;
        }
    ...
    /**
     * Increase the faction points depending on level PK Kills give half the
     * points of a PVP Kill
     */
    ...
    public Map<Integer, Integer> getPvpPkPenaltyes() {
        return _pvppk_penalties;
    }

    public void setPvpPkPenaltyes(Map<Integer, Integer> penalty) {
        _pvppk_penalties = penalty;
    }
    ...
    public void increasePvpKills(int level) {
        ...
        if (Config.ALLOW_PVP_REWARD
            ...
            try {
                ...
                if (hasPvpRewardPenalty(_pvppk_penalties.get(_nowKilled))) {
                    return;
                }

                if (Config.PVP_CHECK_HWID || Config.PVP_CHECK_IP) {
                    if (getHWid() == null) {
                        return;
                    }
                    ...
                    if (Config.PVP_CHECK_HWID && _client.isSameHWID(viktim.getHWid(), getHWid())) {
                        return;
                    }
                    if (Config.PVP_CHECK_IP && viktim.getIP().equalsIgnoreCase(getIP())) {
                        return;
                    }
                }
    ...
    private boolean hasPvpRewardPenalty(Integer last) {
        if (Config.PVPPK_STEP == 0) {
            return false;
        }
        if (last == null) {
            _pvppk_penalties.put(_nowKilled, 1);
            return false;
        }
        if (last > Config.PVPPK_STEP) {
            _lastReward = System.currentTimeMillis() + Config.PVPPK_STEPBAN;
            _pvppk_penalties.clear();
            return true;
        }
        _pvppk_penalties.put(_nowKilled, last + 1);
        return false;
    }
...
//		stopAutoSaveTask();
    ...
    private L2PcInstance _transactionRequester = null;

    public void setTransactionRequester(L2PcInstance requester) {
        _transactionRequester = requester;
    }

    public L2PcInstance getTransactionRequester() {
        return _transactionRequester;
    }
    ...
    public void cancelActiveTrade(boolean f) {
        if (_activeTradeList == null) {
            return;
        }
        setTrading(false);
     ...
     * to the L2PcINstance then return true.<BR><BR>
     */
    public boolean mount(L2Summon pet) {
        ...
        if (isNoStrider()) {
            sendMessage("В данной местности запрещено садиться страйдера.");
            return false;
        }
        ...
        for (L2Effect e : getAllEffects()) {
            ...
            if (e.getSkill().isToggle() && e.getSkill().getId() != 5399) {
                e.exit();
            }
        }
        ...
        if (Config.EPIC_STRIDER_SLOW && getBossZone() >= 1) {
            SkillTable.getInstance().getInfo(4258, 1).getEffects(this, this);
        }
        return true;
    }
    ...
    public final boolean dismount() {
        ...
        if (setMount(0, 0, 0)) {
            ...
            // Notify self and others about speed change
            broadcastUserInfo();
            if (Config.EPIC_STRIDER_SLOW) {
                stopSkillEffects(4258);
            }
            return true;
        }
        return false;
    ...
    public boolean isInvul() {
        return _isInvul || _isTeleporting || isProtected();
    }
    ...
    public static L2PcInstance load(int objectId) {
        return load(objectId, 0);
    }

    public static L2PcInstance load(int objectId, int lang) {
        ...
        try {
            ...
            if (rset.next()) {
                ...
                player._hwidLast = rset.getString("hwid");
                ...
                //System.out.println("###2#" + rset.getInt("base_class"));
            ...
            player.setUptime(System.currentTimeMillis());
            //
            player.giveItems(con, objectId);
            player.checkMail(con, player);

            player.setInBossLogout(OfflineManager.getInstance().isInBossLogout(objectId));
        } catch (Exception e) {
    ...
    private int _emailNotify = 0;

    private static void checkMail(Connection con, L2PcInstance player)
    {
        try
        {
            con = L2DatabaseFactory.getInstance().getConnection();Throwable localThrowable3 = null;
            try
            {
    ...
    private void giveItems(Connection con, int charId) {
        ResultSet rs = null;
        ...
        try {
            tb = new TextBuilder();
            boolean find = false;
            stm = con.prepareStatement("SELECT item_id, count, enchant_level FROM character_items WHERE owner_id=? AND status=?");
            stm.setInt(1, charId);
            stm.setInt(2, 55);
            rs = stm.executeQuery();
            while (rs.next()) {
                find = true;
                String act = "(Player: " + getName() + "(" + charId + "), hwid: " + getHWid() + "); itemId: " + rs.getInt("item_id") + "(" + rewardItem(getInventory().addItem("external", rs.getInt("item_id"), rs.getInt("count"), this, null), rs.getInt("enchant_level")).getObjectId() + "); itemCount: " + rs.getInt("count") + "; enchant: " + rs.getInt("enchant_level");
                //Log.add(act, "give_items/EnterWorld");
                tb.append(act + "\n");
                if (Config.notifyGiveItem(rs.getInt("item_id"), rs.getInt("count"))) {
                    Log.add(act, "warnings/giveItems_EnterWorld_" + rs.getInt("item_id"));
                }
            }
            rs.close();
            stm.close();
            if (find) {
                stm = con.prepareStatement("DELETE FROM character_items WHERE owner_id=? AND status=?");
                stm.setInt(1, charId);
                stm.setInt(2, 55);
                stm.execute();
                stm.close();
                Log.add(tb.toString(), "give_items/EnterWorld");
                tb.clear();
                tb = null;
            }
        } catch (SQLException e) {
            _log.warn("L2PcInstance: giveItems() error: " + e);
        } finally {
            try {
                if (rs != null) {
                    rs.close();
    ...
    private L2ItemInstance rewardItem(L2ItemInstance item, int enchant) {
        if (enchant <= 0) {
            return item;
        }
    ...
    public synchronized void store(boolean items, int call) {
        ...
        try {
            ...
            try {
                ...
                _log.error("L2PcInstance: Error saving character " + getName() + " buffs, bugload possible", e);
                e.printStackTrace();
            }
            try {
                storeCharSub(con);
            ...
            try {
                storeBuffProfiles(con);
            } catch (Exception e) {
                _log.error("L2PcInstance: Error saving character " + getName() + " buff profiles, not fatal", e);
            }
    ...
    private void storeCharBase(Connection con) throws SQLException {
        //System.out.println("###1#" + getBaseClass());
        long totalOnlineTime = _onlineTime;
        if (_onlineBeginTime > 0) {
            totalOnlineTime += (System.currentTimeMillis() - _onlineBeginTime) / 1000;
        }
        ...
        statement.setInt(53, getAppearance().getSex() ? 1 : 0);
    ...
    private void storeEffect(Connection con) {
        if (!Config.STORE_SKILL_COOLTIME) {
            return;
        }

        int buff_index = 0;
        List<Integer> done = new FastList<>();
        FastList<StoredSkill> storedSkills = new FastList<>();
        for (L2Effect effect : getAllEffects()) {
            if (effect == null) {
                continue;
            }
            ...
            if (done.contains(skillId)) {
                continue;
            }

            if (effect.getSkill().isAugment()) {
                continue;
            }
            ...
            if (effect.getInUse()) {
                ...
                if (ds != null && ds.isInReuse()) {
                    reuse_delay = ds.delay;
                    systime = ds.expire;
                    _disabledSkills.remove(skillId);
                }
                storedSkills.add(new StoredSkill(skillId, effect.getSkill().getLevel(), effect.getCount(), effect.getTime(), reuse_delay, systime, 0, buff_index));
            }
        }
        ...
        for (Map.Entry<Integer, DisabledSkill> entry : _disabledSkills.entrySet()) {
            ...
            storedSkills.add(new StoredSkill(id, -1, -1, -1, ds.delay, ds.expire, 1, buff_index));
        }

        BuffStoreManager.storeEffects(getObjectId(), getClassIndex(), storedSkills);
        /*PreparedStatement st = null;
         ...
         st = con.prepareStatement("REPLACE INTO character_skills_save (charId,skill_id,skill_level,effect_count,effect_cur_time,reuse_delay,systime,restore_type,class_index,buff_index) VALUES (?,?,?,?,?,?,?,?,?,?)");
     ...
     */
    public void restoreEffects(Connection con_ex) {

        FastList<StoredSkill> storedSkills = BuffStoreManager.getStoredSkills(getObjectId(), getClassIndex());
        if (storedSkills == null || storedSkills.isEmpty()) {
            restoreEffects2(con_ex);
            return;
        }

        L2Skill skill = null;
        StoredSkill storedSkill = null;
        long current = System.currentTimeMillis();
        SkillTable stbl = SkillTable.getInstance();

        for (FastList.Node<StoredSkill> n = storedSkills.head(), end = storedSkills.tail(); (n = n.getNext()) != end;) {
            storedSkill = n.getValue();
            if (storedSkill == null) {
                continue;
            }
            int skillId = storedSkill.skill_id;
            int effectCount = storedSkill.effect_count;
            int effectCurTime = storedSkill.effect_cur_time;
            int restore_type = storedSkill.restore_type;

            long remainingTime = storedSkill.systime - current;
            if (remainingTime > 0) {
                disableSkill(skillId, storedSkill.reuse_delay, (int) remainingTime);
            }
            ...
            int skillLvl = storedSkill.skill_level;
        ...
        updateEffectIcons();
        if (con_ex == null) {
            broadcastUserInfo(); // дубликат setActiveClass(int classIndex)
        }
    }

    public void restoreEffects2(Connection con_ex) {
        ...
        try {
            ...
            st = con.prepareStatement(RESTORE_SKILL_SAVE);
            st.setInt(1, getObjectId());
            st.setInt(2, getClassIndex());
            ...
            while (rset.next()) {
                ...
                long remainingTime = rset.getLong("systime") - current;
                if (remainingTime > 0) {
                    disableSkill(skillId, rset.getInt("reuse_delay"), (int) remainingTime);
                }
                // Just incase the admin minipulated this table incorrectly :x
                if (skillId == -1 || effectCount == -1 || effectCurTime == -1 || restore_type == 1) {
                    continue;
    ...
    /**
     * Метод определения автоматической атаки Оптимизирован 10.03.2010
     *
     */
    @Override
    public boolean isAutoAttackable(L2Character attacker) {
        ...
        if (getAllyId() != 0 && attacker.getAllyId() != 0 && getAllyId() == attacker.getAllyId()) {
            return false;
        }
    ...
    /**
     * Метод использования скила Оптимизирован 10.03.2010
     *
     */
    public void useMagic(L2Skill skill, boolean forceUse, boolean dontMove) {
        ...
        if (_restrictSkills.contains(skill.getId())) {
            sendMessage("Запрещено использовать " + skill.getName() + " в данной зоне!");
            sendPacket(ActionFailed.STATIC_PACKET);
            return;
        }
        ...
        if (Config.BF_TEST && !skill.isOffensive() && hasIllegalTarget()) {
            sendMessage("Запрещено, когда баффер в таргете.");
            sendPacket(ActionFailed.STATIC_PACKET);
            return;
        }
    ...
    /**
     * Метод проверки гтовности к использования магии Оптимизирован 10.03.2010
     *
     */
    private boolean checkUseMagicConditions(L2Skill skill, boolean forceUse, boolean dontMove) {
        ...
        if (sklType == L2SkillType.SUMMON_FRIEND && getKnownList().findDoorInRange(250)) {
            sendMessage("Нельзя призывать вблизи дверей.");
            sendPacket(ActionFailed.STATIC_PACKET);
            return false;
        }
        ...
        if (isInOlympiadMode()) {
            if (skill.isWeddingSkill()) {
                sendMessage("Скилл не доступен для использования на олимпиаде.");
                sendPacket(ActionFailed.STATIC_PACKET);
                return false;
            }
            if (skill.isOlyProtected() && _olyDisabled.contains(skill.getId())) {
                sendMessage("Скилл не доступен для повторного использования на олимпиаде.");
                sendPacket(ActionFailed.STATIC_PACKET);
                return false;
            }
            _olyDisabled.add(skill.getId());
        }
        ...
        if (sklType == L2SkillType.SWEEP && !target.canBeSweepped(this)) {
            return false;
        }
    ...
    public boolean checkPvpSkill(L2Object obj, L2Skill skill, boolean srcIsSummon) {
        ...
        if (isInsideZone(L2Zone.FLAG_PVP) || target.isInsideZone(L2Zone.FLAG_PVP)) {
            if (isInOlympiadMode() && target.isInOlympiadMode()) {
                return true;
            }
            if (skill.isPvpSkill() || (skill.isAuraSkill() && skill.isSkillTypeOffensive())) // pvp skill
            {
        ...
        if (!(isInDuel() && target.getDuelId() == getDuelId()) && // self is not in a duel and attacking opponent
                !isInsideZone(L2Zone.FLAG_PVP) && // Pc is not in PvP zone
                !target.isInsideZone(L2Zone.FLAG_PVP) // target is not in PvP zone
                ) {
    ...
    public boolean setMount(int npcId, int npcLevel, int mountType) {
        switch (mountType) {
            ...
            case 1:
                ...
                if (isNoble()) {
                    addSkill(SkillTable.getInstance().getInfo(325, 1)); // not saved to DB
                }
                break;
            case 2:
                setIsFlying(true);
     ...
     * pitch > 0:looks up,pitch < 0:looks down<BR> time:faster that small value
     * is.<BR>
     */
    ...
    public boolean canDuel() {
        ...
        if (isInsideZone(L2Zone.FLAG_PVP) || isInsideZone(L2Zone.FLAG_PEACE) || isInsideZone(L2Zone.FLAG_CASTLE) || SiegeManager.getInstance().checkIfInZone(this)) {
        ...
        public void run() {
            try {
                ...
                if (food != null && isHungry()) {
                    ...
                    if (handler != null) {
                        handler.useItem(L2PcInstance.this, food, 0);
    ...
    public boolean modifySubClass(int classIndex, int newClassId) {
        ...
        try {
            ...
            statement.execute();
            statement.close();
            BuffStoreManager.removeStoredEffects(getObjectId(), classIndex);
    ...
    public Map<Integer, SubClass> getSubClasses() {
        if (_subClasses == null) {
            _subClasses = new FastMap<Integer, SubClass>().shared("L2PcInstance._subClasses");
        }
    ...
    public synchronized boolean setActiveClass(int classIndex) {
        ...
        if (getCubics() != null && !getCubics().isEmpty()) {
            for (L2CubicInstance cubic : getCubics().values()) {
                if (cubic == null) {
                    continue;
                }
...
//		startAutoSaveTask();
    ...
    /**
     * Function is used in the PLAYER, calls snoop for all GMs listening to this
     * player speak.
     *
    ...
    public void deleteMe() {
        ...
        if (isOfflineTrade() || (Config.DISCO_MOD && _bossZone == 1 && !_bossLogout)) {
            try {
                stopWarnUserTakeBreak();
//				stopAutoSaveTask();
                stopWaterTask();
            ...
            if (Config.DISCO_MOD && _bossZone == 1) {
                _bossLogout = true;
                broadcastUserInfo();
                OfflineManager.getInstance().addBossLogout(getObjectId());
            }
            return;
        }
        ...
        if (isCensor()) {
            try {
                GmListTable.getInstance().deleteModer(this);
            } catch (Exception e) {
            }
        }

        try {
            LogoutManager.getInstance().storeItems(this, _inventory);
        } catch (Exception e) {
        }
        ...
        try {
            if (_pvppk_penalties != null) {
                FloodProtector.storePvPPkPenaltyes(getObjectId(), _pvppk_penalties);
            }
        } catch (Exception e) {
        }
    ...
    private Map<Integer, TimeStamp> _reuseTimeStamps = new SingletonMap<Integer, TimeStamp>().shared("L2PcInstance._reuseTimeStamps");
    ...
    public boolean canOpenPrivateStore() {
        if (getPrivateStoreType() == L2PcInstance.STORE_PRIVATE_NONE) {
            for (L2PcInstance pc : getKnownList().getKnownPlayersInRadius(20)) {
                if (pc == null) {
                    continue;
                }

                if (pc.getPrivateStoreType() != L2PcInstance.STORE_PRIVATE_NONE) {
                    sendMessage("Встаньте подальше от других торговцев.");
                    return false;
                }
            }
        }
        ...
        if (getPet() != null) {
            sendMessage("Отзовите пета.");
            return false;
        }

        if (_event != null) {
            sendMessage("Вы зарегистрированы на ивенте.");
            return false;
        }

        if (isParalyzed() || isAlikeDead() || isMounted() || isCursedWeaponEquipped()) {
            return false;
        }

        if (isProcessingRequest() || isProcessingTransaction()) {
            return false;
        }

        if (isInOlympiadMode() || getObservMode() != 0 || Olympiad.getInstance().isRegistered(this) || Olympiad.getInstance().isRegisteredInComp(this)) {
            return false;
        }
...
//	private ScheduledFuture<?> _autoSaveTask;
    ...
    private abstract class ConditionListener {
        ...
        private final Map<Func, Boolean> _values = new SingletonMap<Func, Boolean>().shared("L2PcInstance.ConditionListener._values");
    ...
    public void showUserMessages() {
        ...
        if (_noble && Config.ALT_OLY_ENABLED && Config.OLY_DATE_ANNOUNCE) {
            sendCritMessage("Дата окончания периода Олимпиады: " + new SimpleDateFormat("dd.MM.yyyy HH:mm").format(Olympiad.getInstance().getOlympiadEnd()));
        }

        if (isTopPvp() && Config.TOP_PVP_ANNOUNCE) {
            Announcements.getInstance().announceToAll("Лучший игрок PvP - " + getName() + " вошел в игру!");
        }

        if (_emailNotify == 1) {
            sendPacket(ExMailArrived.STATIC_PACKET);
            if (_lang == 0) {
                sendCritMessage("У вас есть посылка на почте! Поспешите :)");
            } else {
                sendCritMessage("You got parcel! Talk with NPC 'Mails'.");
            }
        }

        if (Config.HWID_STATUS_SHOW) {
            String whid = "";
            try {
                whid = getAccountData().getString("hwbind");
            } catch (IllegalArgumentException e) {
                getAccountData().set("hwbind", "");
            }
            sendCritMessage("Статус привязки по железу: " + (whid.length() > 0 ? "Включено" : "Выключено"));
        }

        if (Config.TOP_CLAN_MOD) {
            ServerData.getInstance().showTopClans(this, null);
        }

        checkChatBanDelay(ObjectRestrictions.getInstance().getChatRestricion(this, AvailableRestriction.PlayerChat));
    }

    private void checkChatBanDelay(long delay) {
        if (delay == 0) {
            return;
        }

        if (_lang == 0) {
            sendCritMessage("Бан чата истекает через " + ((delay - System.currentTimeMillis()) / 60000) + " минут.");
        } else {
            sendCritMessage("Chat is disabled for: " + ((delay - System.currentTimeMillis()) / 60000) + " minutes.");
        }
    }
     ...
     *
     * @param indirect as Boolean - true выдать окно NpcHtml, false - поместить
     * в очередб сообщений
     */
    ...
    /**
     * Метод служит для включения всех активных скилов Обнуляет значения
     * пре-подготовки к использованию
     *
     */
    ...
    public void sendSkillCoolTime(boolean force) {
        sendPacket(new SkillCoolTime(this, getDisabledSkills()));
    }

    /**
     * Возвращает boolean значение Проверка принадлежит ли класс чара к списку
     * самонеров
     *
     */
    ...
    /**
     * Вовзращает boolean значение Показывать/ Скрывать трейдеров в knownlist
     *
     */
    ...
    /**
     * Устанавливает значение _showTraders Для внешней конфигурации
     *
     */
    ...
    /**
     * Возвращает boolean значение Находится ли чар в состоянии торка, скупки,
     * крафта
     *
     */
    ...
    /**
     * Возвращает переенную boolean Не показывать анимации бафа
     *
     */
    ...
    /**
     * Устанавливает значение для _showBuffAnimation Для внещней конфигурации
     *
     */
    ...
    /**
     * Возвращает переенную boolean Автолут включен / выключен
     *
     */
    public boolean isAutoLootEnabled(int itemId) {
        if (ItemTable.isStackable(itemId) && getInventory().getItemByItemId(itemId) != null) {
            return true;
        }
        if (!getInventory().validateCapacity(0)) {
            return false;
        }

        return isAutoLootEnabled();
    }
    ...
    /**
     * Устанавливает значение для _useAutoLoot Для внещней конфигурации
     *
     */
    ...
    public void loadSetting(Connection con) throws SQLException {
        ...
        try {
            ...
            //_log.error("Cant get nameColor for player " + getName() + " " + e);
        ...
        try {
            ...
            // _log.error("Cant get titleColor for player " + getName() + " " + e);
            _characterData.set("titleColor", getAppearance().getTitleColor());
        }
    }

    /**
     * Сохранение личных настроек персонажа Данные настроек меню персонажа
     */
    ...
    public int getTitleColor() {
        ...
        if (getAppearance() == null) {
            System.out.println("#getTitleColor##");
    ...
    public int getNameColor(L2PcInstance cha) {
        if (_event != null && _event.isRunning()) {
            return _event.getCharNameColor(this);
        }
        return getNameColor();
    }
    ...
    public int getNameColor() {
        ...
        if (isTopPvp() && Config.TOP_PVP_COLOR > 0) {
            return Config.TOP_PVP_COLOR;
        }
    ...
    public boolean canGainExp() {
        /*if (QueenAntManager.getInstance().isPlayerInside(getObjectId())) {
         return false;
         }*/

        if (_bossZone == 1 && !Config.BOSS_ZONE_EXP) {
            return false;
        }
    ...
    public boolean isPreventedFromReceivingBuffs() {
        ...
        return super.isPreventedFromReceivingBuffs();
    }

    @Override
    public boolean isPreventedFromReceivingBuffs(L2Character caster, boolean ignore) {
        if (getFirstEffect(1411) != null) {
            return true;
        }
        ...
        if (getFirstEffect(Config.BLOCK_BUFF_ID) != null) {
            return true;
        }

        if (getParty() != null && getParty().getMemberById(caster.getObjectId()) != null) {
            return false;
        }

        return isPreventedFromReceivingBuffs();
    }
    ...
    private boolean _fsZone = false;

    @Override
    public void setFsZone(boolean f) {
        _fsZone = f;
    }

    @Override
    public boolean isInFsZone() {
        return _fsZone;
    }
    private ConcurrentLinkedQueue<Integer> _olyDisabled = new ConcurrentLinkedQueue<Integer>();

    public void clearOlyReuses() {
        _olyDisabled.clear();
    }
    private boolean _noSummon = false;
    ...
    public boolean isNoSummon() {
        return _noSummon || (Config.FORBID_AOE_SUMMON && isInAoeZone());
    }
    private boolean _noStrider = false;

    @Override
    public void setNoStrider(boolean f) {
        _noStrider = f;
    }

    public boolean isNoStrider() {
        return _noStrider;
    }
    private boolean _olyZOne = false;

    @Override
    public void setOlyZone(boolean f) {
        _olyZOne = f;
    }

    public boolean isInOlyZone() {
        return _olyZOne;
    }

    public boolean isInOly() {
        if (isInOlympiadMode() || Olympiad.getInstance().isRegistered(this) || getOlympiadGameId() != -1) {
            return true;
        }
        return false;
    ...
    public void checkUserKey(String key) {
        if (key.length() > 1) {
            ...
            /*if (_hwidLast == null || getClient().getHWid() == null) {
             _userKey.on = 0;
             } else {
             _userKey.on = _hwidLast.equals(getClient().getHWid()) ? 0 : 1;
             }*/
            if (Config.HWID_IGNORE_CHARKEY
                    && getClient().isHwidBinded()
                    && _hwidLast.equals(getClient().getHWid())) {
                _userKey.on = 0;
            } else {
                _userKey.on = 1;
            }
    ...
    public void setUserKey(String key) {
        if (Config.charkey_enable && getCharKey()) {
            if (_userKey.key.equalsIgnoreCase(key)) {
                return;
            }

            if (Config.CHAR_KEY_PREFIX) {
                key = key + "" + Rnd.get(10, 99);
            }

            String keyPrint = key;
            key = StringUtil.md5(Config.CHAR_KEY_SALT + key + Config.CHAR_KEY_SALT2);

            _characterData.set("charkey", key);
            _userKey.key = key;
            unsetUserKey();
            store(false, 10);

            NpcHtmlMessage SET_KEY_OK = new NpcHtmlMessage(0);
            SET_KEY_OK.setFile("data/html/menu_set_form_ok" + (_lang == 1 ? "_en" : "") + ".htm");
            SET_KEY_OK.replace("%KEY%", keyPrint);
            sendPacket(SET_KEY_OK);
            SET_KEY_OK = null;
        }
    }
    ...
    public boolean isValidUserKey(String key) {
        key = StringUtil.md5(Config.CHAR_KEY_SALT + key + Config.CHAR_KEY_SALT2);
        return key.equalsIgnoreCase(_userKey.key);
    }
    ...
    public void unsetUserKey() {
        _userKey.on = 0;
        if (getClient().getExpiredPWD() == 0) {
            sendHtmlMessage("Пароль устарел.");
        }
    }
    ...
    public void setUserKeyOnLevel() {
        if (Config.charkey_enable && getCharKey()) {
            ...
            SET_CHAR_KEY.setFile("data/html/menu_set_form" + (_lang == 1 ? "_en" : "") + ".htm");
            sendPacket(SET_CHAR_KEY);
            SET_CHAR_KEY = null;
        }
    }
    private String _phoneNumber = "";
    private String _phoneNumberTmp = "";
    private String _phoneUnblock = "";
    private int _phoneUnblockType = 0;
    private int _smsSended = 0;
    ...
    public void setPhoneNumber(String phone) {
        ...
        _phoneNumber = phone;
        _client.setPhone(_phoneNumber);
        LoginServerThread.getInstance().setPhone(getAccountName(), _phoneNumber, getHWid());

        NpcHtmlMessage SET_PHONE_OK = new NpcHtmlMessage(0);
        SET_PHONE_OK.setFile("data/html/phone_set_form_ok.htm");
        SET_PHONE_OK.replace("%PHONE%", phone);
        sendPacket(SET_PHONE_OK);
        SET_PHONE_OK = null;
    }

    public void setCheckPhoneNumber(String phone) {
        _phoneUnblockType = 99;
        _phoneNumberTmp = phone;
        _phoneUnblock = String.valueOf(Rnd.get(100000, 999999));
        NpcHtmlMessage SET_PHONE = new NpcHtmlMessage(0);
        SET_PHONE.setFile("data/html/phone_ublock_notify.htm");
        sendPacket(SET_PHONE);
        SET_PHONE = null;
        SmsEngine.sendSMS(this, _phoneNumberTmp, _phoneUnblock);
    }
    ...
    public void setPhoneOnLevel() {
        if (!_phoneNumber.isEmpty()) {
            return;
        }

        NpcHtmlMessage SET_PHONE = new NpcHtmlMessage(0);
        SET_PHONE.setFile("data/html/phone_set_form.htm");
        if (Config.SMS_SEND_FORCE) {
            SET_PHONE.replace("%NEXT_TIME%", "");
        } else {
            SET_PHONE.replace("%NEXT_TIME%", "<a action=\"bypass -h voice_kphone nexttime\" msg=\"Пусть лучше ваш аккаунт взломают?\">Нет, спасибо!</a><br>");
        }
    ...
    public void sendUnblockSMS(int code) {
        if (_phoneNumber.isEmpty()) {
            return;
        }

        _smsSended = 1;
        _phoneUnblock = String.valueOf(code);
        SmsEngine.sendSMS(this, _phoneNumber, _phoneUnblock);
    }
    ...
    public boolean checkSMSCode(String code) {
        ...
        return _phoneUnblock.equalsIgnoreCase(code);
    }

    public void unblockSMSGuard() {
        switch (_phoneUnblockType) {
            case 1:
                sendHtmlMessage("HWID добавлен!");
                _client.addHwid(getHWid());
                LoginServerThread.getInstance().addNewHWID(getAccountName(), getHWid());
                break;
            case 99:
                sendHtmlMessage("Разблокирован!");
                setPhoneNumber(_phoneNumberTmp);
                break;
        }
        _smsSended = 0;
        _phoneUnblock = "";
    ...
    public void notifyHwidLock() {
        _phoneUnblockType = 1;
        _phoneUnblock = String.valueOf(Rnd.get(100000, 999999));
    }

    private void setSMSUblock() {
        switch (_phoneUnblockType) {
            case 1: //hwid
                if (_smsSended == 0) {
                    NpcHtmlMessage HWID = new NpcHtmlMessage(0);
                    HWID.setFile("data/html/phone_ublock_form.htm");
                    sendPacket(HWID);
                    HWID = null;
                } else {
                    NpcHtmlMessage SET_PHONE = new NpcHtmlMessage(0);
    ...
    public void checkUserKey() {
        if(Config.charkey_enable && getCharKey()) {
            try {
                checkUserKey(_characterData.getString("charkey"));
            } catch (IllegalArgumentException e) {
                _characterData.set("charkey", "");
            }
            ...
            if (_userKey.on == 1) {
                ...
                UNBLOCK_CHAR_KEY.setFile("data/html/menu_unblock_form" + (_lang == 1 ? "_en" : "") + ".htm");
                sendPacket(UNBLOCK_CHAR_KEY);
                UNBLOCK_CHAR_KEY = null;
            } else if (Config.VS_CKEY_CHARLEVEL && getLevel() >= 0) {
                setUserKeyOnLevel();
            } else if (Config.SMS_PHONE && getLevel() >= Config.SMS_SEND_CHAR_LEVEL) {
                setPhoneOnLevel();
            } else {
                sendPwdExpire();
            }
    ...
    private void sendPwdExpire() {
        if (getClient() == null || Config.PASSWORD_EXPIRE == 0) {
            return;
        }
        if (!isExpiredPwd(getClient().getExpiredPWD())) {
            return;
        }
        NpcHtmlMessage pex = new NpcHtmlMessage(0);
        pex.setFile("data/html/pass_expired.htm");
        sendPacket(pex);
        pex = null;
    }

    private boolean isExpiredPwd(int days) {
        if (Config.PASSWORD_EXPIRE == 0) {
            return false;
        }
        return days == - 1 || days > Config.PASSWORD_EXPIRE;
    }
    ...
    public boolean isParalyzed() {
        ...
        if(Config.charkey_enable && getCharKey()){
            if (_userKey.on == 1 && !_userKey.key.isEmpty()) {
                ...
                if (_userKey.error == 1) {
                    ...
                    UNBLOCK_CHAR_KEY.setFile("data/html/menu_unblock_form_error" + (_lang == 1 ? "_en" : "") + ".htm");
        ...
        } else if (_phoneUnblockType >= 1) {
            setSMSUblock();
        } else if (Config.VS_CKEY_CHARLEVEL && _userKey.key.isEmpty() && getLevel() >= 0) {
            setUserKeyOnLevel();
        } else if (isNeedSetSMS()) {
            setPhoneOnLevel();
        } else if (_userKey.on == 0) {
            sendPwdExpire();
        }
     ...
     * @return
     */
    public boolean getCharKey() {
        try {
            return _characterData.getBool("char_key");
        } catch (IllegalArgumentException var2) {
            _characterData.set("char_key", false);
            return false;
        }
    }
     ...
     * @return
     */
    public void setCharKey(boolean var) {
        _characterData.set("char_key", var);
    }

    private boolean isGuarded() {
        return (_userKey.on == 1
                || super.isParalyzed()
                || isExpiredPwd(getClient().getExpiredPWD())
                || isNeedSetSMS()
                || _phoneUnblockType >= 1);
    }

    private boolean isNeedSetSMS() {
        if (!Config.SMS_SEND_FORCE) {
            return false;
        }

        if (getLevel() < Config.SMS_SEND_CHAR_LEVEL) {
            return false;
        }
    ...
    public class BuffProfile {
        ...
        public String name;
        public List<Integer> _buffs = new FastList<Integer>();

        public BuffProfile(String name) {
            this.name = name;
        }
    }
    private Map<String, BuffProfile> _buffprofiles = new FastMap<String, BuffProfile>();

    public Map<String, BuffProfile> getBuffProfiles() {
        return _buffprofiles;
    }

    public void putBuffProfile(String profileName) {
        _buffprofiles.put(profileName, new BuffProfile(profileName));
    }

    public void putBuffProfile(String profileName, BuffProfile profile) {
        _buffprofiles.put(profileName, profile);
    }

    public void clearBuffProfiles() {
        _buffprofiles.clear();
    }

    private void restoreBuffProfiles(Connection con) {
        _buffprofiles.clear();
        ResultSet rs = null;
        PreparedStatement stm = null;
        try {
            stm = con.prepareStatement("select * from character_buff_profiles where charId=? order by profileName");
            stm.setInt(1, getObjectId());
            rs = stm.executeQuery();
            String profileName = "";
            BuffProfile profile = null;
            while (rs.next()) {
                String current = rs.getString("profileName");
                if (current.equals(profileName)) {
                    ...
                    profileName = current;
                    profile = new BuffProfile(profileName);
                    profile._buffs.add(rs.getInt("buffGroup"));
                }
            }
            rs.close();
    ...
    private void storeBuffProfiles(Connection con) {
        if (_buffprofiles.isEmpty()) {
            return;
        }
        PreparedStatement stm = null;
        try {
            stm = con.prepareStatement("delete from character_buff_profiles where charId=?");
            stm.setInt(1, getObjectId());
            stm.execute();
            stm.close();

            con.setAutoCommit(false);
            for (Map.Entry<String, BuffProfile> entry : _buffprofiles.entrySet()) {
                String profileName = entry.getKey();
                BuffProfile profile = entry.getValue();
                if (profileName == null || profile == null) {
                    continue;
                }

                stm = con.prepareStatement("REPLACE INTO character_buff_profiles VALUES (?,?,?)");
                stm.setInt(1, getObjectId());
                stm.setString(2, profileName);
                for (int buffs : profile._buffs) {
                    stm.setInt(3, buffs);
                    stm.addBatch();
                }
                stm.executeBatch();
    ...
    public boolean updateAccPassword(String pass, String old) {
        pass = StringUtil.getSHA1(pass);
        if (pass.length() < 5) {
            return false;
        }
        old = StringUtil.getSHA1(old);

        getClient().setPWD(old);
        getClient().setRemainPWD(Config.PASSWORD_EXPIRE);

        LoginServerThread.getInstance().setNewPassword(getAccountName(), pass, StringUtil.md5(old));
        return true;
    }
    ...
    public long getItemCountLong(int itemId) {
        return Long.valueOf(checkItemCount(getInventory().getItemByItemId(itemId)));
    }
    ...
    private String _lastSayString = "2w";
    ...
    public boolean identSay(String text) {
        ...
        if (text.equalsIgnoreCase(_lastSayString) || text.startsWith(_lastSayString)) {
            ...
            if (_lastSayCount >= Config.PROTECT_SAY_COUNT) {
                ...
                try {
                    ObjectRestrictions.getInstance().addRestriction(this, AvailableRestriction.PlayerChat);
                    ObjectRestrictions.getInstance().timedRemoveRestriction(this.getObjectId(), AvailableRestriction.PlayerChat, Config.PROTECT_SAY_BAN, "Блокировка чата снята.");
                } catch (RestrictionBindClassException e) {
                    _lastSayTime = System.currentTimeMillis() + Config.PROTECT_SAY_BAN;
                    _lastSayCount = 0;
                    return true;
                }
            }
    ...
    public void updateLastMessPm() {
        ...
        if (_lastMessCountPM > Config.SAY_PM_COUNT) {
            markSpamer();
            /*Log.add(getFingerPrints() + ":" + _lastMessPM, "pm_protect");
             switch (Config.SAY_PM_BAN_TYPE) {
             case 1:
             try {
             ObjectRestrictions.getInstance().addRestriction(this, AvailableRestriction.PlayerChat);
             ObjectRestrictions.getInstance().timedRemoveRestriction(this.getObjectId(), AvailableRestriction.PlayerChat, Config.SAY_PM_BAN_TIME, "Блокировка чата снята.");
             } catch (RestrictionBindClassException e) {
             _lastMessTimePM = System.currentTimeMillis() + Config.SAY_PM_INTERVAL;
             _lastMessCountPM = 0;
             }
             ...
             break;
             case 2:
             BanManager.getInstance().banHwid(null, this);
             break;
             }*/
        }
    }
    ...
    public String getEventTitle(L2PcInstance other) {
        if (other == null || other._event == null) {
            return getTitle();
        }
        if (_event == null) {
            return getTitle();
        }
        return _event.getCharTitle(this);
    }

    public String getEventName(L2PcInstance other) {
        if (other == null || other._event == null) {
            return getName();
        }
        if (_event == null) {
        ...
        return _event.getCharName(this);
    }

    public int getEventNameColor(L2PcInstance other) {
        if (other == null || other._event == null) {
            return getNameColor();
        }
        if (_event == null) {
    ...
    public int getEventTitleColor(L2PcInstance other) {
        if (other == null || other._event == null) {
        ...
        return _event.getCharTitleColor(this);
    }

    public boolean canUseAtEvent(L2ItemInstance item) {
        if (_event == null) {
            return true;
        }
        ...
        return _event.canUseItem(this, item);
    }
    private int _hennaClassId = 0;

    public void setHennaClass() {
        _hennaClassId = _classIndex;
    }

    public boolean wrongHennaClass() {
        return _hennaClassId != _classIndex;
    }
    ...
    public void changeNickName(String name) {
        ...
        sendMessage("Ваш ник изменен на " + name);
        ...
        if (_party != null) {
            _party.refreshPartyView();

            teleToLocation(getX(), getY(), getZ());

            refreshMail(getName(), name);
            Hero.getInstance().updateHeroName(getObjectId(), name);
        }
    }

    private void refreshMail(String old, String name) {
        Connection con = null;
        PreparedStatement st = null;
        try {
            con = L2DatabaseFactory.getInstance().getConnection();

            st = con.prepareStatement("UPDATE `z_post_in` SET `from`=? WHERE `from`=?");
            st.setString(1, name);
            st.setString(2, old);
            st.execute();
            st.close();
            st = con.prepareStatement("UPDATE `z_post_in` SET `to`=? WHERE `to`=?");
            st.setString(1, name);
            st.setString(2, old);
            st.execute();
            st.close();

            st = con.prepareStatement("UPDATE `z_post_out` SET `from`=? WHERE `from`=?");
            st.setString(1, name);
            st.setString(2, old);
            st.execute();
            st.close();
            st = con.prepareStatement("UPDATE `z_post_out` SET `to`=? WHERE `to`=?");
            st.setString(1, name);
            st.setString(2, old);
            st.execute();
            st.close();

            st = con.prepareStatement("UPDATE `z_post_pos` SET `from`=? WHERE `from`=?");
            st.setString(1, name);
            st.setString(2, old);
            st.execute();
            st.close();
            st = con.prepareStatement("UPDATE `z_post_pos` SET `to`=? WHERE `to`=?");
            st.setString(1, name);
            st.setString(2, old);
            st.execute();
            st.close();

            L2FriendList.updateFriendName(this, con);
        } catch (SQLException e) {
            _log.error("refreshMail from " + e);
        } finally {
            try {
                if (con != null) {
                    con.close();
    ...
    @Override
    public void sendGmMessage(String string) {
        if (!isGM()) {
            return;
        }
        sendMessage(string);
    ...
    public int getChatLanguage() {
        ...
        try {
            return getAccountData().getInteger("chat");
        } catch (IllegalArgumentException e) {
            setChatLanguage(0);
            return 0;
        }
    ...
    public void setChatLanguage(int lang) {
        _lang = lang;
        getAccountData().set("chat", _lang);
    }
    //
    private boolean _shotPacket = true;

    @Override
    public boolean isShowShotPacket() {
        return _shotPacket;
    }

    public void setShowShotPacket(boolean f) {
        _shotPacket = f;
    }

    @Override
    public void sendSoulShotPacket(final L2GameServerPacket mov) {
        if (!_shotPacket) {
            return;
        }
        sendPacket(mov);
    ...
    @Override
    public double getStatModifier(Stats param) {
        if (getOlympiadSide() != -1) {
            return Formulas.getCharModifierOlymp(this, param);
        }
        return Formulas.getCharModifier(this, param);
    }
    private long _lastPartyPosTime = 0;

    public boolean canSendPartyPos() {
        if (_lastPartyPosTime > System.currentTimeMillis()) {
            return false;
        }

        _lastPartyPosTime = System.currentTimeMillis() + 30000;
        return true;
    }

    @Override
    public int getTeam() {
        return checkTeam(super.getTeam());
    }

    private int checkTeam(int team) {
        if (team == 0 && isTopPvp() && Config.TOP_PVP_AURA > 0) {
            return Config.TOP_PVP_AURA;
        }
        return team;
    }

    public boolean isTopPvp() {
        if (!Config.TOP_PVP_MOD) {
            return false;
        }
        return ServerData.getInstance().isPlayerBestPvp(getObjectId());
    }
    //
    private long _lastMovePacket = 0;
    private int _lastMovePacketCount = 0;

    public boolean canUseMovePacket() {
        if (_lastMovePacket > System.currentTimeMillis()) {
            if (_lastMovePacketCount >= Config.JPMTEST1) {
                _lastMovePacketCount = 0;
                _lastMovePacket = System.currentTimeMillis() + Config.JPMTEST2;
                return false;
            }
            _lastMovePacketCount++;
            return false;
        }

        _lastMovePacketCount = 0;
        _lastMovePacket = System.currentTimeMillis() + Config.JPMTEST3;
        return true;
    }

    public void disableMove(int delay) {
        _lastMovePacket = System.currentTimeMillis() + delay;
    }
    private long _lastActionPacket = 0;
    private int _lastActionPacketCount = 0;

    public boolean canUseActionPacket() {
        if (_lastActionPacket > System.currentTimeMillis()) {
            if (_lastActionPacketCount >= Config.JPMTEST1) {
                _lastActionPacketCount = 0;
                _lastActionPacket = System.currentTimeMillis() + Config.JPMTEST2;
                return false;
            }
            _lastActionPacketCount++;
            return false;
        }

        _lastActionPacketCount = 0;
        _lastActionPacket = System.currentTimeMillis() + Config.JPMTEST3;
        return true;
    }

    @Override
    public void disableActions(int delay) {
        _lastActionPacket = System.currentTimeMillis() + delay;
        _lastMovePacket = System.currentTimeMillis() + delay;
    }
    //
    ...
    //
    private long _lastQhpUse = 0;
    private long _lastTrade = 0;

    public boolean canUseQHP() {
        if (_lastQhpUse > System.currentTimeMillis()) {
            return false;
        }

        _lastQhpUse = System.currentTimeMillis() + Config.QHP_DELAY;
        return true;
    }

    public boolean canAcceptTrade() {
        if (_lastTrade > 0 && _lastTrade < System.currentTimeMillis()) {
            _lastTrade = 0;
            return false;
        }

        _lastTrade = System.currentTimeMillis() + 10000;
        return true;
    }
    ...
    public boolean isInTradeRequest() {
        return _lastTrade > System.currentTimeMillis();
    }

    public void setTradeExpire() {
        _lastTrade = System.currentTimeMillis() + 10000;
    }
    //
    ...
    public boolean isCensor() {
        if (_cmoder) {
            ...
            try {
                con = L2DatabaseFactory.getInstance().getConnection();
                st = con.prepareStatement("SELECT isCensor FROM `characters` WHERE `charId`=? LIMIT 1");
                st.setInt(1, getObjectId());
                result = st.executeQuery();
                if (result.next()) {
                    _moder = result.getInt("isCensor") == 1;
                }
            } catch (final SQLException e) {
                _log.warn("isCensor() error: " + e);
            } finally {
                try {
                    if (result != null) {
                        result.close();
    ...
    private int _effectErrorsCount = 0;

    @Override
    public void checkEffectErrors() {
        if (_effectErrorsCount >= 3) {
            try {
                new Disconnection(this).defaultSequence(false);
            } catch (Exception e) {
                _log.warn("checkEffectErrors() error: " + e);
            }
        }
        _effectErrorsCount++;
    }
    ...
    public void sendCritMessage(String text) {
        sendPacket(new CreatureSay(0, SystemChatChannelId.Chat_Critical_Announce, "", text));
    }

    public void sendAdmMessage(String text) {
        sendPacket(new CreatureSay(0, SystemChatChannelId.Chat_Normal, "SYS", text));
    }
    //
    ...
    public DecodedBypass decodeBypass(String bypass) {
        ...
        IBBSHandler handler = CommunityBoard.getInstance().getHandler(bypass);
        if (handler != null) {
    ...
    public double getCollisionRadius() {
        ...
        return getBaseTemplate().getdCollisionRadius();
    }
    ...
    public double getCollisionHeight() {
        ...
        return getBaseTemplate().getdCollisionHeight();
    }
    ...
    public static L2PcInstance restoreFake(int objectId, int classId, boolean summon) {
        ...
        player = new L2PcInstance(objectId, template, "fake_qwerty", app);
    ...
    public void setSpamer(boolean f) {
        _spamer = f;
    }

    public void sendChatMessage(String ru, String en) {
        sendMessage(_lang == 0 ? ru : en);
    }
    ...
    private long _lastCastPacket = 0;
    private int _lastCastPacketCount = 0;

    public boolean canUseCastPacket(int skillId) {
        if (isSkillDisabled(skillId)) {
            return false;
        }
        //sendMessage("###1#" + (_lastCastPacket - System.currentTimeMillis()) + "##" + _lastCastPacketCount);
        if (_lastCastPacket > System.currentTimeMillis()) {
            if (_lastCastPacketCount >= Config.CPTEST1) {
                _lastCastPacketCount = 0;
                _lastCastPacket = 0;//System.currentTimeMillis() + Config.CPTEST2;
                //System.out.println("##Ooops!##");
                disableSkill(skillId, Config.CPTEST2);
                sendSkillCoolTime();
                return false;
            }
            _lastCastPacketCount++;
            return false;
        }

        _lastCastPacketCount = 0;
        _lastCastPacket = System.currentTimeMillis() + Config.CPTEST3;
        return true;
    }
    private boolean _bossLogout = false;

    @Override
    public void setInBossLogout(boolean f) {
        _bossLogout = f;
    }

    @Override
    public boolean isInBossLogout() {
        return _bossLogout;
    }
    private Location _flagLoc = null;

    @Override
    public void setFlagLoc(Location loc) {
        _flagLoc = loc;
    }
    ...
    public boolean isInFlagZone() {
        return (_flagLoc != null);
    }
    //
    private int _farmDelay = 0;
    private int _farmLesson = 0;
    private int _farmTryes = 0;
    private String _farmLink = "";

    public void showFarmDelayWindow() {
        _farmLink = "farm_bot" + Rnd.get(10000, 99921);
        _farmDelay = Rnd.get(10000, 99921);
        _farmLesson = _farmDelay;
        NpcHtmlMessage html = new NpcHtmlMessage(0);
        switch (Config.FARM_CHECK_TYPE) {
            case 0:
                TextBuilder tb = new TextBuilder("<br>");
                for (int i = (Rnd.get(30)); i > 0; i--) {
                    tb.append("<br>");
                }
                html.setHtml("<html><body><font color=\"FF6600\">!Активирован штраф на награду с мобов!</font><br>Нажмите на кнопку для снятия штрафа!<br> <table width=\"" + Rnd.get(40, 300) + "\"><tr><td align=\"right\">" + tb.toString() + "<button value=\"Продолжить\" action=\"bypass -h " + _farmLink + " " + _farmDelay + "\" width=80 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td></tr></table></body></html>");
                tb.clear();
                tb = null;
                break;
            case 1:
                html.setHtml("<html><title>UniquePvP.ru [Запрет на Дроп]</title><body><center><img src=\"uniquepvp.logo\" width=256 height=90></center>"
                        + "<center><a action=\"\"> <font color=\"FF6600\">Штраф на награду с мобов!</font></a><br>"
                        + "<img src=\"uniquepvp.rovno\" width=256 height=12 align=left></center><br>"
                        + "<center><a action=\"\"> <font color=\"969696\">Введите код:</font></a><br></center>"
                        +"<center><button value=\"===>"+ _farmDelay + "<===\" width=121 height=21 back=\"uniquepvp.3knbig_over\" fore=\"uniquepvp.3knbig_over\"></center><br><br>"
                        + "<center><table><tr><td><edit var=\"pwd\" width=100 length=\"10\"></td><td><button value=\"Ввести\" action=\"bypass -h "
                        + _farmLink + " $pwd\" width=79 height=21 back=\"uniquepvp.2kn\" fore=\"uniquepvp.2kn_over\"></td></tr></table></center><br><br>"
                        + "<center><img src=\"uniquepvp.rovno\" width=256 height=12 align=left></center></body></html>");
            ...
            case 2:
                ...
                int b = Rnd.get(100);
                _farmLesson = a + b;
                html.setHtml("<html><body><font color=\"FF6600\">!Активирован штраф на награду с мобов!</font><br>Решите пример,<br>=== <font color=LEVEL> " + a + " + " + b + " = ?</font>, ===<br> для снятия штрафа: <br><edit var=\"pwd\" width=60 length=\"5\"><br><br><button value=\"Ok\" action=\"bypass -h " + _farmLink + " $pwd\" width=40 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></body></html>");
                break;
            case 3://test
                html.setHtml("<html><body><font color=\"FF6600\">!Активирован штраф на награду с мобов!</font><br>Введите этот код,<br><center><img src=\"CAPTCHA.12171217\" height=\"35\" width=\"128\"></center><br><br><edit var=\"pwd\" width=60 length=\"5\"><br><br><button value=\"Ok\" action=\"bypass -h farm_bot $pwd\" width=40 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></body></html>");
                break;
            case 4:
                int aa = 12;
                int ab = 12;
                ...
                html.setHtml("<html><title>UniquePvP.ru [Запрет на Дроп]</title><body>"
                        + "<center><img src=\"uniquepvp.logo\" width=256 height=90></center>"
                        + "<center><a action=\"\"> <font color=\"FF6600\">Штраф на награду с мобов!</font></a>"
                        + "<br><a action=\"\"> <font color=" + color + ">Какого цвета этот текст?</font></a>"
                        + "<center><img src=\"uniquepvp.rovno\" width=256 height=12 align=left></center><br>"
                        + "<br><a action=\"bypass -h " + _farmLink + " " + aa + "\"><font color=" + aac + ">[UniquePvP.ru]</font></a>"
                        + "<br><a action=\"bypass -h " + _farmLink + " " + ab + "\"><font color=" + abc + ">[UniquePvP.ru]</font></a>"
                        + "<br><a action=\"bypass -h " + _farmLink + " " + ac + "\"><font color=" + acc + ">[UniquePvP.ru]</font></a>"
                        + "<br><a action=\"bypass -h " + _farmLink + " " + ad + "\"><font color=" + adc + ">[UniquePvP.ru]</font></a>"
                        + "<br><a action=\"bypass -h " + _farmLink + " " + ae + "\"><font color=" + aec + ">[UniquePvP.ru]</font></a>"
                        + "</center><br><center><img src=\"uniquepvp.rovno\" width=256 height=12 align=left></center></body></html>");
                break;
        }
        sendPacket(html);
        /*_farmTryes++;
    ...
    public void showFarmDelayOk() {
        _farmDelay = 0;
        _farmLesson = 0;
        _farmTryes++;
        //_farmTryes = 0;
        sendHtmlMessage("Спасибо", "Запрет снят.");
        ServerData.getInstance().clearFarmDelays(getObjectId());
    }

    @Override
    public boolean hasFarmDelay(L2Attackable mob) {
        if (getBossZone() > 0) {
            return false;
        }

        if (mob != null && mob.passFarmDelayCheck()) {
            return false;
        }

        if (_farmTryes >= Config.FARM_CHECK_IGNORE) {
            return false;
        }
        return _farmDelay > 1;
    }
    ...
    public String getFarmLink() {
        return _farmLink;
    }

    public boolean isGoodFarmer() {
        return (_farmTryes >= Config.FARM_CHECK_IGNORE);
    }
    //
    //_lastMovePacket
    private long _lastSayMove = 0;
    private int _lastSayMoveCount = 0;

    public void setLastSayMove() {
        _lastSayMove = _lastMovePacket;
    }

    public boolean isSayMoveEquals() {
        return (_lastSayMove == _lastMovePacket);
    }

    public boolean isSayMoveFull() {
        if (_lastSayMoveCount > Config.CHAT_MOVE_CHECK_COUNT) {
            _lastSayMoveCount = 0;
            return true;
        }
        _lastSayMoveCount++;
    ...
    private boolean _aqZone = false;

    @Override
    public void setAqZone(boolean f) {
        _aqZone = f;
    }

    public boolean isInAqZone() {
        return _aqZone;
    }
    ...
    public void useAutoItem(int itemId)
    {
        ...
        if ((Config.ACP_SS) && (Config.ACP_ENGINE)) {
            Potions.getInstance().useItem(this, item, 0);
        } else {
            handler.useItem(this, item, 0);
        }
    }
    ...
    private FastList<Integer> _restrictSkills = new FastList<Integer>();

    @Override
    public void setRestricts(FastList<Integer> skills) {
        _restrictSkills = skills;
    }

    @Override
    public void emptyRestricts() {
        _restrictSkills.clear();
    }

    public boolean hasIllegalTarget() {
        ...
        return (npc.getNpcId() == 50000);
    }
    private int _questKey = 0;

    public int getQuestKey() {
        return _questKey;
    }

    public void setQuestKey() {
        _questKey = Rnd.get(10000, 99999);
    }
    ...
    //
    private String _unblockCode = "";

    public void setUnblockCode(String code) {
        _unblockCode = code;
    }

    public String getUnblckCode() {
        return _unblockCode;
    }

    public void teleToLocation(Location loc) {
        teleToLocation(loc.getX(), loc.getY(), loc.getZ());
    }

    //
    private boolean _olyPrepare;

    public void setOlyPrepare(boolean f) {
        _olyPrepare = f;
        if (f) {
            disableAllSkills();
            return;
    ...
    public final boolean isFalling(int z) {
        if (isDead() || isFlying() || isInWater() || isInsideZone(L2Zone.FLAG_WATER)) {
            return false;
        }
        ...
        if (deltaZ <= 500/*getBaseTemplate().getSafeFallHeight()*/) {
            return false;
        }
    ...
    public void restoreCharSettings(L2PcInstance player)
    {
        SimpleDateFormat dateFormat = new SimpleDateFormat("dd.MM.yyyy в HH:mm:ss");
        boolean sucess = false;
        Connection con = null;
        try {
            con = L2DatabaseFactory.getInstance().getConnection();
            PreparedStatement statement = con.prepareStatement(CHAR_SETTINGS);
            statement.setInt(1, player.getObjectId());
            ResultSet rset = statement.executeQuery();
            while (rset.next()) {
                sucess = true;
                ...
                if (enddate <= System.currentTimeMillis()) {
                    ...
                    player.setHero(true);
                    player.sendCritMessage("Геройство закончится " + dateFormat.format( new Date(enddate) ));
                }
            }
            statement.close();
        } catch (Exception e) {
    ...
    public void restorePremSettings(L2PcInstance player)
    {
        SimpleDateFormat dateFormat = new SimpleDateFormat("dd.MM.yyyy в HH:mm:ss");boolean sucess = false;Connection con = null;
        try
        {
            con = L2DatabaseFactory.getInstance().getConnection();
            PreparedStatement statement = con.prepareStatement("SELECT valueData FROM account_data WHERE account_name=? AND valueName=?");
            ...
            {
                sucess = true;
                long valueData = rset.getLong("valueData");
                if (valueData <= System.currentTimeMillis())
                {
                    if (valueData == 0L) {}
                }
                else {
                    player.sendCritMessage("Премиум закончится " + dateFormat.format(new Date(valueData)));
                }
            }
        ...
        {
            _log.error("PremService: Could not restore PremiumStatus data for " + player.getName() + ": ", e);
        }
        finally
        {
            try
     ...
     * @return
     */
    public int getACP_CP() {
        try {
            return _characterData.getInteger("acp_cp");
        } catch (IllegalArgumentException var2) {
            _characterData.set("acp_cp", Config.ACP_CP);
            return Config.ACP_CP;
        }
    }
     ...
     * @return
     */
    public int getACP_HP() {
        try {
            return _characterData.getInteger("acp_hp");
        } catch (IllegalArgumentException var2) {
            _characterData.set("acp_hp", Config.ACP_HP);
            return Config.ACP_HP;}}
     ...
     * @return
     */
    public int getACP_MP() {
        try {
            return _characterData.getInteger("acp_mp");
        } catch (IllegalArgumentException var2) {
            _characterData.set("acp_mp", Config.ACP_MP);
            return Config.ACP_MP;}}
     ...
     * @return
     */
    public void setACP_MP(int var) {
        if(var >= 0 && var <= 100)
            _characterData.set("acp_mp", var);
        else
            sendMessage("Не верно задан параметр!");
     ...
     * @return
     */
    public void setACP_HP(int var) {
        if(var >= 0 && var <= 100)
            _characterData.set("acp_hp", var);
        else
            sendMessage("Не верно задан параметр!");
     ...
     * @return
     */
    public void setACP_CP(int var) {
        if(var >= 0 && var <= 100)
            _characterData.set("acp_cp", var);
        else
            sendMessage("Не верно задан параметр!");

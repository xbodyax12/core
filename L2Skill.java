...
public class L2Skill implements FuncOwner {
    ...
    private final double _powerOly;
    ...
    private boolean _isVortex = false;
    private boolean _isOlyProtected = false;
    private boolean _isPvpSkill = false;
    private boolean _isAuraTarget = false;
    private boolean _ignoreBlockBuff = false;
    private boolean _isStoreStat = false;
    private boolean _isSlowDebuff = false;
    private boolean _checkSaBuff = true;
    private boolean _isWeddingSkill = false;
    ...
    private boolean _isForcedAugment = false;
    private boolean _isCancelProtected = false;
    private boolean _isShowMessages = true;
    private boolean _isShowCastMessages = true;
    private boolean _isIgnoreMagicLaunch = false;
    private boolean _isMassTarget = false;
    private boolean _isWalkBuff = false;
    private boolean _isShieldBuff = false;
    private boolean _isForbidOly = false;
    ...
    public L2Skill(StatsSet set) {
        ...
        if (isToggle() && Config.TOGGLE_REUSE_DELAY > 0) {
            _reuseDelay = Config.TOGGLE_REUSE_DELAY;
        } else {
        ...
        _powerOly = set.getFloat("powerOly", 0.f);
        ...
        //_isAugment
        if (_id >= 3080 && _id <= 3299) {
            _isAugment = true;
            if (isSkillTypeOffensive()) {
                _isForcedAugment = true;
            }
        }
        //vortex
        if (_id >= 1338 && _id <= 1343) {
            _isVortex = true;
        }

        //_isOlyProtected костыль, пока не расскажут как работает баг
        /*switch (_id) {
         case 341:
         case 342:
         case 406:
         _isOlyProtected = true;
         break;
         }*/
        //_isPvpSkill
        switch (_skillType) {
        ...
        _ignoreBlockBuff = Config.IGNORE_BUFFS.contains(_id);
        if (_isPvpSkill) {
            _ignoreBlockBuff = true;
        }
        ...
        switch (_id) {
            ...
            case 4515:
            case 5182:
                _isCancelProtected = true;
                break;
            default:
                _isCancelProtected = _isMalariaBuff;
                break;
        }
        ...
        //_isForbidOly
        if (Config.FORBIDDEN_OLY_SKILLS.contains(_id)) {
            _isForbidOly = true;
        }
    }
    ...
    public boolean isWalkBuff() {
        return _isWalkBuff;
    }

    public boolean isShieldBuff() {
        return _isShieldBuff;
    }

    public boolean showAnimToOthers(L2PcInstance player) {
        if (player == null) {
            return true;
        }

        if (_checkFirstBuff
                && Config.MAGIC_BOSSZONE_PACKET
                && _isMassTarget
                && player.isInBossZone()) {
            return false;
        }
    ...
    public boolean showMagicLaunch(L2PcInstance player) {
        if (player == null) {
            return true;
        }

        if (Config.IGNORE_AURAF_PACKET && _isIgnoreMagicLaunch) {
            return false;
        }

        if (Config.AURA_BUFF_PACKET && _isMassTarget) {
            return false;
        }

        if (Config.MAGIC_BOSSZONE_PACKET && player.isInBossZone()) {
            return false;
        }
    ...
    public boolean isIgnoreMagicLaunch() {
        if (!Config.IGNORE_AURAF_PACKET) {
            return false;
        }
        return _isIgnoreMagicLaunch;
    }

    public boolean isMassTarget() {
        return _isMassTarget;
    }

    public boolean isShowMessages() {
        return _isShowMessages;
    }

    public boolean isShowCastMessages() {
        return _isShowCastMessages;
    }

    public boolean isCancelProtected() {
        return _isCancelProtected;
    }
    ...
    public boolean isWeddingSkill() {
        return _isWeddingSkill;
    }

    public boolean isSlowDebuff() {
        return _isSlowDebuff;
    }

    public boolean isCheckSaBuff() {
        return _checkSaBuff;
    }

    public boolean isIgnoreBlockBuff() {
        return _ignoreBlockBuff;
    }
    ...
    public final double getPower(L2Character activeChar) {
        if (activeChar.isInOlympiadMode() && _powerOly > 0) {
            return _powerOly;
        }
        return _power;
    }
                        ...
                        if (obj instanceof L2Attackable || obj instanceof L2PlayableInstance) {
                            ...
                            if (src != null) {
                                ...
                                if (obj instanceof L2PcInstance) {
                                    ...
                                    if (src.getParty() != null && src.getParty().getPartyMembers().contains((L2PcInstance) obj)) {
                                        continue;
                                    }
                ...
                case TARGET_BEHIND_AREA: {
                    ...
                    for (L2Object obj : activeChar.getKnownList().getKnownObjects().values()) {
                        ...
                        if (!target.isDead() && (target != activeChar)) {
                            if (!Util.checkIfInRange(getSkillRadius(), obj, cha, true)) {
                                continue;
                            }
                    ...
                    if (activeChar.getParty() != null) {
                        ...
                        for (L2PcInstance partyMember : partyList) {
                            ...
                            if (!partyMember.isDead() && Util.checkIfInRange(getSkillRadius(), activeChar, partyMember, true)) {
                                if (getSkillRadius() > 0 && !GeoData.getInstance().canSeeTarget(activeChar, partyMember)) {
                                    continue;
                                }
...
/*								if (getSkillType() == L2SkillType.RESURRECT)
                                ...
                                if (radius > 0 && !GeoData.getInstance().canSeeTarget(activeChar, newTarget)) {
                                    continue;
                                }
                ...
                case TARGET_AREA_CORPSE_MOB: {
                    ...
                    for (L2Object obj : activeChar.getKnownList().getKnownObjects().values()) {
                        ...
                        if (!Util.checkIfInRange(getSkillRadius(), target, obj, true)) {
                            continue;
                        }

                        if (src != null && (trg = obj.getPlayer()) != null) {
                            ...
                            if (!srcInArena && !(((L2Character) obj).isInsideZone(L2Zone.FLAG_PVP) && !((L2Character) obj).isInsideZone(L2Zone.FLAG_SIEGE))) {
                                if (src.getAllyId() == trg.getAllyId() && src.getAllyId() != 0) {
                                    continue;
                                }
                ...
                case TARGET_AURA_CORPSE_MOB: {
                    ...
                    if (activeChar.getKnownList() != null) {
                        for (L2Object obj : activeChar.getKnownList().getKnownObjects().values()) {
                            ...
                            if (!cha.isDead() || !Util.checkIfInRange(radius, activeChar, cha, true)) {
                                continue;
                            }
    ...
    public synchronized final L2Effect[] getEffects(L2Character effector, L2Character effected, IEffector effectorObj) {
        ...
        if (effector != effected) {
            if (effected.isInvul()
                    || effector.getOlympiadGameId() != effected.getOlympiadGameId()) {
                return L2Effect.EMPTY_ARRAY;
            }
        }
    ...
    public boolean isLikeDebuff() {
        return _isDebuff || _isMalariaBuff;
    }

    public boolean isMalariaBuff() {
    ...
    public boolean isOlyProtected() {
        return _isOlyProtected;
    }
    ...
    public boolean isForcedAugment() {
        return _isForcedAugment;
    }
